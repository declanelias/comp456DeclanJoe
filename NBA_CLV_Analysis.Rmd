...
load("NBA_movement_split_df.rdata")
```
...
NBA_Full = subset(NBA_Full, select = -c(id,book_id, league, Location) )
...
NBA_cv10 <- vfold_cv(NBA_Full, v = 10)
...
# Recipe
logistic_rec <- recipe(make_bet ~ money_line + spread + total + team_total, data = NBA_Full, family = binomial('logit'), maxit = 100) %>%
...
log_fit <- fit(log_wf, data = NBA_Full)
```
...
predict(log_fit, new_data = NBA_Full, type = "prob")

# Make hard (class) predictions (using a default 0.5 probability threshold)
predict(log_fit, new_data = NBA_Full, type = "class")
```
...
```{r}
# Soft predictions
logistic_output <-  NBA_Full %>%
  bind_cols(predict(log_fit, new_data = NBA_Full, type = 'prob')) 
...
# Recipe
data_rec <- recipe(make_bet ~ money_line + spread + total + team_total, data = NBA_Full)
...
data_fit_mtry2 <- fit(data_wf_mtry2, data = NBA_Full)
...
set.seed(121) 
data_fit_mtry4 <- fit(data_wf_mtry4, data = NBA_Full)

"set.seed(121)
data_fit_mtry6 <- fit(data_wf_mtry6, data = NBA_Full)"
```
...
#rf_OOB_output(data_fit_mtry6,6, NBA_Full %>% pull(make_bet))
rf_OOB_output(data_fit_mtry4,4, NBA_Full %>% pull(make_bet))
rf_OOB_output(data_fit_mtry2,2, NBA_Full %>% pull(make_bet))
```
...
prediction <- predict(fit_mod, new_data = NBA_Full, type = "class")
```

```{r}
table_mat <- table(NBA_Full$make_bet, prediction$.pred_class)
table_mat
```
```{r}
