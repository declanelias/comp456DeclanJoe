...
NFL_Full = subset(NFL_Full, select = -c(id,book_id, league, Location) )
...
# Recipe
logistic_rec <- recipe(make_bet ~ money_line + spread + total + team_total, data = NFL_Full, family = binomial('logit'), maxit = 100) %>%
...
# Recipe
data_rec <- recipe(make_bet ~ spread + total + team_total + money_line, data = NFL_Full)
...
set.seed(121) 
data_fit_mtry4 <- fit(data_wf_mtry4, data = NFL_Full)

"set.seed(121)
data_fit_mtry6 <- fit(data_wf_mtry6, data = NFL_Full)"
```
...
mtry4Conf <- rf_OOB_output(data_fit_mtry4,4, NFL_Full %>% pull(make_bet)) %>%
    conf_mat(truth = make_bet, estimate= .pred_class)
...
```{r}
NFL_Full <- NFL_Full %>%
  select(-public_ml, -public_spread, -public_team_over)
```
```{r}
NFL_Full <- NFL_Full %>%
  select(-outcome)
...
```{r}
TidyForest <- tidyRF(model_output, NFL_Train, NFL_Test)
```
...
data_rec <- recipe(make_bet ~ money_line + spread + total + team_total , data = NFL_Full)
