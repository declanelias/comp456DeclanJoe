...
load("NHL_movement_split_df.rdata")
```
...
NHL_Full = subset(NHL_Full, select = -c(id,book_id, league, Location) )
...
NHL_cv10 <- vfold_cv(NHL_Full, v = 10)
...
# Recipe
logistic_rec <- recipe(make_bet ~ money_line + spread + total + team_total, data = NHL_Full, family = binomial('logit'), maxit = 100) %>%
...
log_fit <- fit(log_wf, data = NHL_Full)
```
...
predict(log_fit, new_data = NHL_Full, type = "prob")

# Make hard (class) predictions (using a default 0.5 probability threshold)
predict(log_fit, new_data = NHL_Full, type = "class")
```
...
```{r}
# Soft predictions
logistic_output <-  NHL_Full %>%
  bind_cols(predict(log_fit, new_data = NHL_Full, type = 'prob')) 
...
# Recipe
data_rec <- recipe(make_bet ~ money_line + spread + total + team_total, data = NHL_Full)
...
data_fit_mtry2 <- fit(data_wf_mtry2, data = NHL_Full)
...
set.seed(121) 
data_fit_mtry4 <- fit(data_wf_mtry4, data = NHL_Full)

"set.seed(121)
data_fit_mtry6 <- fit(data_wf_mtry6, data = NHL_Full)"
```
...
#rf_OOB_output(data_fit_mtry6,6, NHL_Full %>% pull(make_bet))
rf_OOB_output(data_fit_mtry4,4, NHL_Full %>% pull(make_bet))
rf_OOB_output(data_fit_mtry2,2, NHL_Full %>% pull(make_bet))
```
...
prediction <- predict(fit_mod, new_data = NHL_Full, type = "class")
```

```{r}
table_mat <- table(NHL_Full$make_bet, prediction$.pred_class)
table_mat
```
```{r}
