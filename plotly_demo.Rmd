...
set.seed(123)
tips_split <- initial_split(tips, prop = 3/4)
tips_training <- tips_split %>%
  training()
tips_test <- tips_split %>%
  testing()
...
tips$Table_Gender <- paste(tips$split,tips$sex)
```
...
tips_split <- initial_split(tips, prop = 3/4)
train_data <- training(tips_split)
test_data <- testing(tips_split)
...
mesh_size = .02
margin = 0.25
x_min =  min(tips$total_bill) - margin
x_max = max(tips$total_bill) + margin
y_min = min(tips$tip) - margin
