...
ncaaf_ml = average_books(ncaaf_raw, "ncaaf")

nba_raw = clean_data("nba")
nba_ml = average_books(ncaaf_raw, "nba")

nba_clean = nba_ml %>% final_clean(2017, 2021, 500)
...
## HAVE TO CLEAN RAW DATA (THIS LOAD LEAGUE FUNCTION LOADS ML_DATA)
nba_ml = load_league("nba") %>% final_clean(2017, 2021, 500)
ncaab_ml = load_league("ncaab") %>% final_clean(2017, 2021, 450)
nhl_ml = load_league("nhl") %>% final_clean(2017, 2021, 900)
nfl_ml = load_league("nfl") %>% final_clean(2017, 2021, 140)
mlb_ml = load_league("mlb") %>% final_clean(2017, 2022, 500)
ncaaf_ml = load_league("ncaaf") %>% final_clean(2018, 2022, 150)

nba_ml = nba_ml %>% select(-c(is_legal))
save(nba_ml, file = "betting_data/nba_ml_clean.rdata")

ncaab_ml = ncaab_ml %>% select(-c(is_legal))
save(ncaab_ml, file = "betting_data/ncaab_ml_clean.rdata")

nhl_ml = nhl_ml %>% select(-c(is_legal))
save(nhl_ml, file = "betting_data/nhl_ml_clean.rdata")

nfl_ml = nfl_ml %>% select(-c(is_legal))
save(nfl_ml, file = "betting_data/nfl_ml_clean.rdata")

mlb_ml = mlb_ml %>% select(-c(is_legal))
save(mlb_ml, file = "betting_data/mlb_ml_clean.rdata")
...
```{r warning=FALSE}
for (league in c("ncaab", "nhl", "mlb")) {
  print(paste("started", league))

  raw = clean_data(league)
  ml = average_books(raw, league)
  save(ml, file = paste("betting_data/", league, "_ml_clean.rdata", sep = ""))
  
  print(paste("finished", league))
}

raw = clean_data("nba")
ml = average_books(raw, "nba")
...
```{r}
get_brier_score(ncaaf_ml %>% filter(season == 2019)) %>%
  arrange(desc(brier_skill_score))

get_brier_score(ncaaf_ml %>% filter(season == 2020)) %>%
  arrange(desc(brier_skill_score))

get_brier_score(ncaaf_ml %>% filter(season == 2021)) %>%
  arrange(desc(brier_skill_score))

get_brier_score(ncaaf_ml %>% filter(season == 2022)) %>%
  arrange(desc(brier_skill_score))

get_brier_score(ncaaf_ml) %>%
  arrange(desc(brier_skill_score))
```


## Using the top Performing Book

#### Create the Testing and Training Set

```{r}
data_caesars = ncaaf_ml %>%
  filter(season == 2022)
```

```{r}
data_caesars = data_caesars %>%
...
## Using the Brier Score as a weighted average
...
levels(logreg_train$home_team_win)
...
confusion_mat = logistic_output  %>%
  conf_mat(truth = home_team_win, estimate = .pred_win)

tn = confusion_mat$table[1]
fn = confusion_mat$table[3]
tp = confusion_mat$table[4]
fp = confusion_mat$table[2]
...
brier_ss = get_brier_ss_of_book(brier_score)
```



```{r}
glmnet(x = private$data_test_long$home_team_win, y = private$data_test_long %>% select(-c(id, home_team_win, brier_score, season)), family = "binomial")
```
