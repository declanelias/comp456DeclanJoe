...
```{r}
save(nfl_df_early, file = "nfl_df_early.rdata")
```
...
for (season in 2017:2021) {
  ...
  for (week in 1:20) {
    ...
    resp_df = jsonlite::fromJSON(content(resp, "text"))$games
    if (length(resp_df) != 0) {
      ncaaf_df_early = rbind(ncaaf_df_early, clean_df_Early(resp_df, week, season))
    }
...
days2018 = format_date(seq(as.Date("2018-03-29"), as.Date("2019-10-28"), by="days"))
