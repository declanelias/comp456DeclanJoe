...
lockBinding("BOOK_ID_DATA", globalenv())
...
# Get Data For Each League DO NOT CHANGE THIS
```{r warning=FALSE}

map(
  c("nfl", "ncaaf", "nhl", "mlb", "ncaab", "nba"),
  function(league) {
    leagues = "all"
    books = "all"
    start_year = 2017
    end_year = 2022
    is_clv = FALSE
    df = get_betting_data(league, books, start_year, end_year, is_clv)
    save(df, file = paste("betting_data/", league, ".rdata",sep = ""))
    ml_df = df %>%
      select(id, away_team_id, home_team_id, winning_team_id, 
             league_name, season, ml_away, ml_home, ml_home_implied_prob, 
