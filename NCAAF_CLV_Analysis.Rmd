...
load("NCAAF_movement_split_df.rdata")
```
...
NCAAF_Full <- NCAAF_Sample %>%
  filter(Location == 'home')

NCAAF_Full = subset(NCAAF_Full, select = -c(id,book_id, league, Location) )
...
# Recipe
logistic_rec <- recipe(make_bet ~ money_line + spread + total + team_total, data = NCAAF_Full, family = binomial('logit'), maxit = 100) %>%
...
log_fit <- fit(log_wf, data = NCAAF_Full)
```
...
predict(log_fit, new_data = NCAAF_Full, type = "prob")

# Make hard (class) predictions (using a default 0.5 probability threshold)
predict(log_fit, new_data = NCAAF_Full, type = "class")
```
...
```{r}
# Soft predictions
logistic_output <-  NCAAF_Full %>%
  bind_cols(predict(log_fit, new_data = NCAAF_Full, type = 'prob')) 
...
# Recipe
data_rec <- recipe(make_bet ~ spread + total + team_total + money_line, data = NCAAF_Full)
...
data_fit_mtry2 <- fit(data_wf_mtry2, data = NCAAF_Full)
...
set.seed(121) 
data_fit_mtry4 <- fit(data_wf_mtry4, data = NCAAF_Full)

"set.seed(121)
data_fit_mtry6 <- fit(data_wf_mtry6, data = NCAAF_Full)"
```
...
#rf_OOB_output(data_fit_mtry6,6, NCAAF_Full %>% pull(make_bet))
rf_OOB_output(data_fit_mtry4,4, NCAAF_Full %>% pull(make_bet))
rf_OOB_output(data_fit_mtry2,2, NCAAF_Full %>% pull(make_bet))
```
...
```
```{r, warning=FALSE}
mtry2Conf <- rf_OOB_output(data_fit_mtry2,2, NCAAF_Full %>% pull(make_bet)) %>%
    conf_mat(truth = make_bet, estimate= .pred_class)

mtry4Conf <- rf_OOB_output(data_fit_mtry4,4, NCAAF_Full %>% pull(make_bet)) %>%
    conf_mat(truth = make_bet, estimate= .pred_class)
...
ct_spec <- decision_tree() %>%
  ...
  set_args(cost_complexity = 0.02,  #default is 0.01 (used for pruning a tree)
...
data_rec <- recipe(make_bet ~ money_line + spread + total + team_total , data = NCAAF_Full)
...
prediction <- predict(fit_mod, new_data = NCAAF_Full, type = "class")
```

```{r}
table_mat <- table(NCAAF_Full$make_bet, prediction$.pred_class)
table_mat
```
