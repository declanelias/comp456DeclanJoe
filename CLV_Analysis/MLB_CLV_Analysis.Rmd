...
MLB_Full = subset(MLB_Full, select = -c(id,book_id, league, Location) )
...
# Recipe
logistic_rec <- recipe(make_bet ~ money_line + spread + total + team_total, data = MLB_Full, family = binomial('logit'), maxit = 100) %>%
...
# Recipe
data_rec <- recipe(make_bet ~ money_line + spread + total + team_total, data = MLB_Full)
...
prediction <- predict(fit_mod, new_data = MLB_Full, type = "class")
```

```{r}
table_mat <- table(MLB_Full$make_bet, prediction$.pred_class)
table_mat
```
```{r}
